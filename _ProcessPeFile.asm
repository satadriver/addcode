...
;********************************************************************
		...
		invoke VirtualAlloc,0,@dwNewFileSize,MEM_COMMIT,PAGE_READWRITE
		mov @lpNewFileBuf,eax
		invoke	SetFilePointer,@hFile,0,NULL,FILE_BEGIN
		lea eax,@dwCount
		invoke ReadFile,@hFile,@lpNewFileBuf,@dwNewFileSize,eax,0

		pushad
		xor eax, eax
		mov ebx, @dwNewFileSize
		mov ecx, ebx
		push ecx
		shr ecx, 1
		mov esi, @lpNewFileBuf
		clc
_checksumPE: 
		adc ax, word ptr [esi]
		inc esi 
...
__endchecksumPE:
		...
		add esi,20
		add esi,64
		mov eax,@dwNewPeCheckSum
		mov dword ptr [esi],eax
		invoke	SetFilePointer,@hFile,0,NULL,FILE_BEGIN
		lea eax,@dwCount
		invoke WriteFile,@hFile,@lpNewFileBuf,@dwNewFileSize,eax,0
		popad
